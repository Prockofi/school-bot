...
class LessonsClass:
    async def main(self, login: str, password: str, school: str, url: str) -> str:
        connect = NetSchoolAPI(url)
    ...
    async def get_data(self, user_id: int, login: str, password: str, school: str, url: str) -> list:
        year = await self.main(login, password, school, url)
        try:
            read_data = (await lessons.get_not_empty(user_id))[1]
        except:
            read_data = "None"
        await lessons.user_id(user_id)
