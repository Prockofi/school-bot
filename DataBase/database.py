...
def db_connect() -> None:
    ...
    cur = db.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, login TEXT, pass TEXT, num_class INT, name_school TEXT, ver INTEGER)")
    cur.execute("CREATE TABLE IF NOT EXISTS lessons(id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, lessons TEXT)")
    db.commit()
...
class DataBase:
    ...
    async def get_user_id(self, user_id: int) -> None:
        try:
